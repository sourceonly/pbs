...
os.environ['ALTAIR_LICENSE_PATH']="6200@login02"
if (debugOn):
        debug_output()
##### Make hostlist
os.system('env')
...
for name in host_list:
  if (hostlist == None):
    hostlist="%s:%d" % (name,nodes[name])
  else:
    hostlist+=",%s:%d" % (name,nodes[name])

print "Your hostlist: %s " % hostlist
...
#  os.environ['MPI_ROOT']="/mnt/nfs1/caesoftware/hpmpi"
  os.environ['PROTO']="IBV"
  os.environ['MPI_REMSH']="/usr/bin/ssh"
  os.environ['P4_RSHCOMMAND']=os.environ['MPI_REMSH']
  os.environ['PBS_RSHCOMMAND']="/usr/bin/ssh"
  os.environ['MPI_CPU_AFFINITY']="ll"
  os.environ['LD_LIBRARY_PATH']="/home/software/HW/11.0/altair/hwsolvers/bin/linux64:/home/software/HW/11.0/altair/hw/bin/linux64/"
...
os.environ['RADIR']=os.path.dirname(os.environ['PAS_EXECUTABLE'])
...
#if (pas_version == "11.0-single"):
#starterbin=os.path.basename(os.environ['PAS_EXECUTABLE'])
#enginebin="e_" + pas_version + '_linux64_plmpi'
#enginebin="e_11.0.240_linux64_plmpi"
#starterbin="s_11.0.240_linux64"
#print radioss_ver
starterbin="s_11.0_linux64_sp"
#enginebin="e_11.0_linux64_plmpi_sp"
#if (pas_version == "11.0-double"):
#starterbin=os.path.basename(os.environ['PAS_EXECUTABLE'])
#starterbin="s_11.0_linux64"
enginebin=os.path.basename(os.environ['PAS_EXECUTABLE'])
...
#print copy_cmd
#os.system(copy_cmd)
copy_cmd="%s %s/%s %s" % ("cp", os.environ['RADIR'], "radflex_11_linux64", ".")
print copy_cmd
os.system(copy_cmd)

cleanup_cmd="rm -f ./radflex_11_linux64"
...
#                          engine_cmd = engine_cmd + " -cpu_bind=v,ll -stdio=i0 -e MPI_FLAGS=y,E"
                          ...
                          engine_cmd = engine_cmd + " -hostlist " + hostlist + " "
                          engine_cmd = engine_cmd + " " + os.environ['RADIR'] + "/" + enginebin
